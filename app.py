#!/usr/bin/env python3
"""
İzin Takip Sistemi - Güvenli Versiyon
...
<title>İzin Takip - Güvenli Giriş</title>
...
<div class="mb-3"><label class="form-label">Çalışan</label>
<select class="form-select" name="employee_name" required>
<option value="">Seçiniz...</option>
{% for emp in employees %}<option value="{{ emp.name }}">{{ emp.name }}</option>{% endfor %}
</select></div>{% endif %}
<div class="mb-3"><label class="form-label">Şifre</label>
<input type="password" class="form-control" name="password" required>
{% if show_default_info %}<small class="text-muted">Varsayılan: admin123</small>{% endif %}
</div>
...
<h2>{{ title }}</h2>
<div>
{% if session.user_type %}<a href="/logout" class="btn btn-outline-danger me-2">Çıkış</a>{% endif %}
<a href="/" class="btn btn-outline-secondary">Ana Sayfa</a>
</div></div>
{% with messages = get_flashed_messages(with_categories=true) %}
{% if messages %}{% for category, message in messages %}
...
if (confirm('⚠️ ' + name + ' silinecek. Emin misiniz?')) {
fetch('/delete_employee', {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, 
body: 'employee_name=' + encodeURIComponent(name)}).then(r => r.ok ? location.reload() : alert('Hata!'));
}}
</script></body></html>
'''
...
def index():
    ...
    <a href="/employee_login" class="btn btn-success btn-lg">👤 İşçi Girişi</a>
    <a href="/employer_login" class="btn btn-warning btn-lg">👔 İşveren Girişi</a>
    </div>
    <small class="text-muted mt-3">Demo şifreler: İşçi=123456, İşveren=admin123</small>
    </div></div>
    ''')
...
def employee_login():
    ...
    return render_template_string(LOGIN_TEMPLATE, title="İşçi Girişi", show_employee_select=True, 
                                employees=employees, show_default_info=False)
...
def employer_login():
    ...
    return render_template_string(LOGIN_TEMPLATE, title="İşveren Girişi", show_employee_select=False, 
                                employees=[], show_default_info=True)
...
def employee_panel():
    ...
    return render_template_string(MAIN_TEMPLATE, title=f"👤 {employee['name']} - İşçi Paneli", content=content)
...
def employer_panel():
    ...
    return render_template_string(MAIN_TEMPLATE, title="👔 İşveren Paneli", content=content)
